...
- (instancetype)initWithText:(NSString *)text subText:(NSString *)subText showSpinner:(BOOL)showSpinner {
    if (self = [super initWithFrame:CGRectZero]) {
        ...
        ZGradientBar *gradient = [[ZGradientBar alloc] init];
        [self addSubview:gradient];

        //Add the zapic logo to the center of the screen
        UIImageView *icon = [[UIImageView alloc] initWithImage:[ZImageUtils getZapicLogo]];
...
- (void)closeButtonAction:(UIButton *)sender {
    if (!_viewController) {
        [ZLog error:@"Unable to close the view. No view controller set"];
        return;
    }
